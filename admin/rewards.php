...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add_reward') {
        ...
        if (empty($name) || $points_required <= 0) {
            ...
            try {
                $stmt = $pdo->prepare("
                    ...
                    VALUES (NULL, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([$name, $description, $points_required, $quantity, $image_url, $status]);
        ...
        if (in_array($status, ['active', 'inactive'])) {
            $stmt = $pdo->prepare("UPDATE rewards SET status = ? WHERE id = ? AND owner_id IS NULL");
            $stmt->execute([$status, $reward_id]);
            $msg = "Cập nhật trạng thái phần quà thành công!";
        }
    } elseif ($_POST['action'] === 'delete_reward') {
        $reward_id = intval($_POST['reward_id']);
        try {
            $stmt = $pdo->prepare("DELETE FROM rewards WHERE id = ? AND owner_id IS NULL");
        ...
        if (in_array($status, ['pending', 'approved', 'rejected', 'delivered'])) {
            try {
                // Kiểm tra xem yêu cầu đổi quà này có phải của quà Admin không
                $stmt_check = $pdo->prepare("
                    SELECT re.id FROM reward_exchanges re
                    JOIN rewards r ON re.reward_id = r.id
                    WHERE re.id = ? AND r.owner_id IS NULL
                ");
                $stmt_check->execute([$exchange_id]);
...
// Lấy danh sách phần quà của Admin
$stmt = $pdo->query("SELECT * FROM rewards WHERE owner_id IS NULL ORDER BY created_at DESC");
$rewards = $stmt->fetchAll();

// Lấy danh sách yêu cầu đổi quà đối với quà của Admin
$stmt_exc = $pdo->query("
...
<div>
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2 style="margin: 0;">Quản lý Quà tặng (Hệ thống)</h2>
            ...
            <div class="card-body">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <input type="text" name="name" required placeholder="VD: Áo đấu đội tuyển Việt Nam" style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px;">
                    </div>
                    ...
                    <div style="display: flex; gap: 15px; margin-bottom: 15px;">
                        <div style="flex: 1;">
                            ...
                            <input type="number" name="points_required" required min="1" placeholder="VD: 500" style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px;">
                        </div>
                        <div style="flex: 1;">
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <select name="status" style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px;">
                            <option value="active">Kích hoạt (Hiển thị cho người dùng)</option>
    ...
    <!-- Danh sách phần quà -->
    <h3 style="margin-bottom: 15px; font-size: 18px; color: var(--dark);">Danh sách phần quà hiện có</h3>
    ...
    <h3 style="margin-bottom: 15px; font-size: 18px; color: var(--dark);">Yêu cầu đổi quà (từ người dùng)</h3>
    <div class="card">
        <div class="card-body" style="padding: 0; overflow-x: auto;">
            <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 14px;">
                <thead>
